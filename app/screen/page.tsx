...
export default function StageScreenPage() {
  ...
  return (
    <div className="h-screen w-screen bg-[#050505] flex overflow-hidden font-sans select-none relative">
        ...
        {match.show_lottery && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-black/85 backdrop-blur-md flex flex-col items-center justify-center font-sans overflow-hidden">
            <motion.div initial={{ scale: 0.8, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} transition={{ type: "spring", bounce: 0.5 }} className="bg-gradient-to-b from-zinc-900 to-black border-4 border-yellow-500/50 p-12 rounded-[3rem] shadow-[0_0_150px_rgba(250,204,21,0.4)] relative flex flex-col items-center min-w-[800px]">
              ...
              {/* 💡 修正 2：給予外框固定的絕對高度 (h-[550px]) 與 relative，子元素絕對定位，保證切換時外框與標題不會跳動 */}
      ...
      {/* 💡 修正 1：QR Code 移動到右側垂直置中，完美對齊藍色方塊 */}
      {!match.show_lottery && originUrl && (
        <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1 }} className="absolute top-1/2 -translate-y-1/2 right-20 z-40 bg-black/80 backdrop-blur-xl p-4 rounded-3xl border border-red-900/50 shadow-[0_0_40px_rgba(0,0,0,0.8)] flex flex-col items-center">
          ...
          <div className="bg-white p-2 rounded-2xl shadow-inner"><QRCode value={originUrl} size={130} level="H" /></div>
        </motion.div>
      )}

...
export default function StreamOverlayPage() {
  ...
  const [lotteryPage, setLotteryPage] = useState(0); // 💡 新增：輪播分頁
  ...
  return (
    <div className="w-[1920px] h-[320px] bg-transparent flex overflow-hidden font-sans select-none drop-shadow-2xl">
      ...
      <div className="flex-1 h-full bg-zinc-950 flex relative overflow-hidden border-t-[4px] border-blue-600 shadow-[10px_0_30px_rgba(0,0,0,0.8)] z-10">
         ...
         <div className="w-[320px] h-full relative shrink-0 z-10">
            <div className="w-full h-full relative flex items-center justify-center">
               {match.p1_avatar && <img src={match.p1_avatar} className="absolute w-[800px] h-[800px] object-contain max-w-none" style={{ transform: `translate(${match.p1_stream_x - 50}%, ${match.p1_stream_y - 50}%) scale(${match.p1_stream_size / 100})` }} alt="p1" />}
            </div>
         </div>
         <div className="flex-1 flex flex-col justify-center items-end px-12 relative z-20">
            ...
            <div className="flex items-baseline gap-6">
              ...
              <div className="flex items-baseline text-white tracking-tighter drop-shadow-[0_0_15px_rgba(37,99,235,0.4)]">
      ...
      <div className="w-[320px] h-full bg-gradient-to-b from-zinc-900 to-black flex flex-col items-center justify-center relative z-30 shadow-[0_0_50px_rgba(0,0,0,1)] border-x border-zinc-800">
         <div className="absolute top-0 w-full h-2 bg-gradient-to-r from-yellow-700 via-yellow-400 to-yellow-700"></div>
         
         <AnimatePresence mode="wait">
           {!match.show_lottery ? (
             <motion.div key="vote" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center w-full">
               <div className="text-[65px] font-black italic text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-400 leading-[0.85] mb-4 text-center tracking-tighter pr-3">LIVE<br/>VOTE</div>
               <div className="bg-yellow-500 text-black font-black px-6 py-1.5 rounded-sm text-sm tracking-[0.2em]">{match.tournament_name || 'SWC 2025'}</div>
             </motion.div>
           ) : (
               ...
               <div className="w-full flex flex-col gap-2 items-center">
                 {currentWinners.map((w: any, idx: number) => (
                   <div key={idx} className="bg-white/10 w-full px-4 py-2 rounded-lg border border-white/20 text-center">
                     <div className="text-white font-black text-lg truncate">{w.user_name}</div>
      ...
      <div className="flex-1 h-full bg-zinc-950 flex flex-row-reverse relative overflow-hidden border-t-[4px] border-red-600 shadow-[-10px_0_30px_rgba(0,0,0,0.8)] z-10">
         ...
         <div className="w-[320px] h-full relative shrink-0 z-10">
            <div className="w-full h-full relative flex items-center justify-center">
               {match.p2_avatar && <img src={match.p2_avatar} className="absolute w-[800px] h-[800px] object-contain max-w-none" style={{ transform: `translate(${match.p2_stream_x - 50}%, ${match.p2_stream_y - 50}%) scale(${match.p2_stream_size / 100})` }} alt="p2" />}
            </div>
         </div>
         <div className="flex-1 flex flex-col justify-center items-start px-12 relative z-20">
            ...
            <div className="flex items-baseline gap-6 flex-row-reverse">
              ...
              <div className="flex items-baseline text-white tracking-tighter drop-shadow-[0_0_15px_rgba(220,38,38,0.4)]">
